...
void Server::AllocMem() {
  ...
  if (use_pm) {
    ...
    hash_buffer = (char*)malloc(hash_buf_size+log_buf_size);
    assert(hash_buffer);
...
void Server::InitMem() {
  ...
  memset(hash_buffer, 0, hash_buf_size+log_buf_size); //DAM allocating extra memory for logs.
...
void Server::InitRDMA() {
  ...
  RDMA_ASSERT(
      rdma_ctrl->register_memory(SERVER_HASH_BUFF_ID, hash_buffer, hash_buf_size+log_buf_size, rdma_ctrl->get_device()) == true);
  RDMA_ASSERT(
