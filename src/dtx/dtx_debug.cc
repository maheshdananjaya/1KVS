...
bool DTX::UndoLogWithoutInserts() {
  ...
  for (auto& set_it : read_write_set) { 
    ...
    if (!set_it.is_fetched && !set_it.is_logged  && !set_it.item_ptr->user_insert ) num_log_entries++;
  }
  ...
  for (auto& set_it : read_write_set) { 
    ...
    if (!set_it.is_fetched && !set_it.is_logged && !set_it.item_ptr->user_insert) {           
      ...
      set_it.is_logged = true; 
      if((t_id == 1))
	      RDMA_LOG(INFO) << "Thread " << new_record.t_id_<< " coro " << coro_id << " , Transaction " << new_record.tx_id_ << " , Logged Key " <<  new_record.data_.key << "  Insert " <<  (set_it.item_ptr->user_insert)? "YES": "NO";  

    }
  }
 
  //RDMA_LOG(DBG) << " starting logs ";

  for(int ll=0 ; ll < num_log_entries; ll++){
	//RDMA_LOG(DBG) << "coro: " << coro_id << " tx_id: " << tx_id << " check read rorw";
	UndoLogRecord * rrr = (UndoLogRecord *) (written_log_buf + (ll * sizeof(UndoLogRecord)));
	assert(rrr != NULL);
  //RDMA_LOG(INFO) << "MEMCPY log record - T_ID " << rrr->t_id_ << " TX ID "  << rrr->tx_id_ << " key logged " << (rrr->data_).key ;
