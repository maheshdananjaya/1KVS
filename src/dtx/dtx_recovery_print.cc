    ...
    for (int t=0; t<num_thread; t++){
        ...
        for (int c = 1 ; c < num_coro ; c++){   
		    ...
		    RDMA_LOG(INFO) << "Record Thread "<< t  <<" , coro " << c << " , record "<< r << " of node " << i << " : tx_id " << record[r].tx_id_ << " , tid  " << record[r].t_id_ << " Key " << record[r].data_.key << " Table Id " << record[r].data_.table_id;
                    ...
                    if(record[r].tx_id_ <= 0){
			    RDMA_LOG (INFO) << "chk1 tx_id is less than zero";
                        if(r==0){
				RDMA_LOG (INFO) << "chk 2 first record";
                            //not even started
                            log_received &= false;  
                            tx_id_agreed &= false;
                            break;            
		    ...
		    RDMA_LOG(INFO) << "Thread " << t << " , Coro " << c << " , has started commited Tx " << coro_agreed_tx_id << " valid logs " << num_valid_logs;	    
            ...
            if(!has_started_commit){
               coro_has_started_commit[t][c] = has_started_commit; ; //TODO- iinvoke unlocks
                ...
                for (int rc =0 ; rc < num_valid_logs; rc++){  //recorc c
                        ...
                        RDMA_LOG(INFO) << "Thread " << t << " , Coro " << c << " , has started commited Tx " <<  logged_key << " of Table " << logged_table_id; 
