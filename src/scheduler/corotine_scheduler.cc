...
//FOR EEL
void CoroutineScheduler::PollRegularCompletion(coro_id_t c_id) {
  for (auto it = pending_qps.begin(); it != pending_qps.end();) {
    RCQP* qp = *it;
    struct ibv_wc wc;
    auto poll_result = qp->poll_send_completion(wc);  // The qp polls its own wc
...
void CoroutineScheduler::PollRegularCompletionForRecovery() {
  for (auto it = pending_qps.begin(); it != pending_qps.end();) {
    RCQP* qp = *it;
    struct ibv_wc wc;
    auto poll_result = qp->poll_send_completion(wc);  // The qp polls its own wc
...
void CoroutineScheduler::PollLogCompletionForRecovery() {
  for (auto it = pending_log_qps.begin(); it != pending_log_qps.end();) {
    RCQP* qp = *it;
    struct ibv_wc wc;
    auto poll_result = qp->poll_send_completion(wc);
...
//for EEL
bool CoroutineScheduler::PollCoro(coro_id_t c_id) {
          if (pending_counts[c_id] == 0) {
                      return true;
                        }
            PollRegularCompletion(c_id);
              return pending_counts[c_id] == 0;
}


//For Recovery
bool CoroutineScheduler::CheckRecoveryDataAck(coro_id_t c_id) {
  if (pending_counts[c_id] == 0) {
    return true;
  }
...
bool CoroutineScheduler::CheckRecoveryLogAck(coro_id_t c_id) {
  if (pending_log_counts[c_id] == 0) {
    return true;
  }
