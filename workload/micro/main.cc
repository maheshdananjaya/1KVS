...
int main(int argc, char* argv[]) {
  ...
  auto thread_arr = new std::thread[thread_num_per_machine + 1+ 1+1]; //+1 is for the additional thread +1 foe failure detector
  ...
  auto* param_arr = new struct thread_params[thread_num_per_machine + 1 +1+1];
  ...
  for (t_id_t i = 0; i < thread_num_per_machine + 1 +1+1 ; i++) {
    ...
    if( i == thread_num_per_machine+1+1 ) thread_arr[i] = std::thread (ZK_HeartBeats, machine_id);
    else if( i == thread_num_per_machine+1 ) thread_arr[i] = std::thread (HeartBeats, machine_id);
